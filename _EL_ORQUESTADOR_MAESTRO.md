...
- **Lote de piezas de Instagram en un HTML (Meta AI):** si el cliente tiene un `01_ADN_y_Memoria/05_prompt_maestro_meta_ai.md`, ese archivo manda — es la especificación (retícula, escala, bloque de estilo, negativos, contrato del HTML). Hoy existe para **Juancito Ads** (lote mensual) y **D'CASA** (semana de contenido). El prompt ya armado se guarda en `03_Redes_Sociales/Instagram_TikTok/` con fecha. Regla del formato: Meta AI genera los fondos y monta el HTML, **nunca escribe el copy**.
